>Yes, this is overkill for our first assignment.

My biggest takeaway from our sole so far 8–2 minute 5013 breakout group activity is the need by real teachers in real classrooms to offload administrative tasks to a cognitive agent.

Coding an app that can take a teacher's swipes, and then filter, calculate and sort anonymized student homework assessments (scores.csv) and report back whichever of the three learning opportunities the teacher chooses to afford:
1. One-on-one remediation tutoring
2. In-class pair-and-share
3. Reteach
